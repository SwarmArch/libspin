...
progenv = env.Clone()
progenv.Append(CPPFLAGS = ['-march=native', '-g', '-std=c++0x'])
progenv.Program(
        target="test",
        source="test.cpp")
...
progenv.Program(
        target="mttest",
        source="mttest.cpp", LIBS = "pthread")

localenv = env.Clone()
localenv.Program(
    target='fast_ctxt.so',
    source="fast_ctxt.cpp")
...
localenv.Append(LIBPATH = [os.path.join(PINPATH, dir) for dir in (
    'extras/xed2-intel64/lib', 'intel64/lib', 'intel64/lib-ext')])
...
localenv.Append(LIBS = ['pin', 'xed', 'elf', 'dl', 'rt',
    File(os.path.join(PINPATH, 'intel64/lib-ext/libdwarf.a'))])
